  ...
  runInit(id: string) {
    ...
    this.service.getRegistration(id).subscribe((data: any) => {
      ...
      if (data.length) {
        ...
        this.displayName = this.info[0];

        if (this.useSquareup) {
          const amt = this.info.amount || this.amount;
          this.amount = parseFloat(amt).toFixed(2);
          this.paidUp = this.info.paid || false;
        }
        if (this.useSquareup && !this.paidUp) { this.loadPaymentForm(); }
    ...
    if (errors) {
      ...
      this.service.runPayment(nonce, this.shortid, this.amount).subscribe(result => {
        ...
        if (status === 200) {
          ...
          this.hasCardErrors = proc_errors;
        }
      }, err => {
        const error = err.json() || [err.message];
        this.hasCardErrors = error;
        this.processingPayment = false;
      });
